...
const Circle = styled.div`
  width:18px; height: 18px;  background-color: white; border: 1px solid rgb(204, 204, 204); border-radius: 100px; display: inline-block; margin-right: 9px;
  &.on{
            ...
            <div>
              {
                Array.from({length: 5}).map((a,i) => {
                  return(
                    <Circle key={i} className={props.info && props.info[type]>= i+1 ? 'on' : ''}/>
                  )
                })
              }
            </div>
